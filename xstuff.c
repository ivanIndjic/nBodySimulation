...
char *theAppName = "NBody Simulation";
Window theMain;
	     ...
	     unsigned width, unsigned height)
{
  ...
  int rc = XAllocNamedColor(theDisplay, default_cmap, "black", &black, &black);
  if (rc == 0) {
  ...
  /* allocate the set of colors we will want to use for the drawing. */
//  rc = XAllocNamedColor(theDisplay, default_cmap, "red", &black, &black);
//  if (rc == 0) {
